import type { CustomCharacter } from '../src/content/types'

import { getDateInt } from '../src/utils/dateint'
export const lawyerCustom: CustomCharacter = {
  name: 'Ayşe',
  type: 'Lawyer',
  image: '/public/lawyer2/lawyer2.svg',
  description: 'Ayşe is a member of the Law Society of Ayrotek looking to access court materials online.',
  revocationInfo: [
    {
      credentialName: 'Person',
      credentialIcon: '/public/lawyer2/icon-lawyer2.svg',
      title: 'Revoke your Person Credential',
      description:
        'Revoking your credentials does not mean that is it unusable. Services may accept revoked credentials especially for information that is unchanging, such as your birthday.',
    },
    {
      credentialName: 'member_card',
      credentialIcon: '/public/lawyer2/icon-lawyer2.svg',
      title: 'Revoke your LSBC Member Card',
      description:
        'The Law Society of Ayrotek allows you to revoke your own Member Card if:\n• there is a problem with your credential.\n• your device was lost or stolen and you want to secure your personal information.',
    },
  ],
  progressBar: [
    {
      name: 'person',
      onboardingStep: 'PICK_CHARACTER',
      iconLight: '/public/common/icon-person-light.svg',
      iconDark: '/public/common/icon-person-dark.svg',
    },
    {
      name: 'moon',
      onboardingStep: 'SETUP_START',
      iconLight: '/public/common/icon-moon-light.svg',
      iconDark: '/public/common/icon-moon-dark.svg',
    },
    {
      name: 'wallet',
      onboardingStep: 'CONNECT_LSBC',
      iconLight: '/public/common/icon-wallet-light.svg',
      iconDark: '/public/common/icon-wallet-dark.svg',
    },
    {
      name: 'notification',
      onboardingStep: 'ACCEPT_PERSON',
      iconLight: '/public/common/icon-notification-light.svg',
      iconDark: '/public/common/icon-notification-dark.svg',
    },
    {
      name: 'balloon',
      onboardingStep: 'SETUP_COMPLETED',
      iconLight: '/public/common/icon-balloon-light.svg',
      iconDark: '/public/common/icon-balloon-dark.svg',
    },
  ],
  onboarding: [
    {
      screenId: 'PICK_CHARACTER',
      title: 'Meet Ayşe',
      text: "Meet Ayşe (that's you in this demo!). Ayşe is a lawyer in good standing with The Law Society of Ayrotek. We know this because she's got her physical Law Society of Ayrotek Member Card. She's also a resident of Ayrotek and has gone through the process to prove her identity with Service and has obtained a physical Card. This has allowed her to use in person services.",
    },
    {
      screenId: 'SETUP_START',
      title: "Let's get started!",
      text: 'AyroID is a new app for storing and using credentials on your smartphone. Credentials are things like IDs, licenses and diplomas. \nUsing your AyroID is fast and simple. In the future it can be used online and in person.You approve every use, and share only what is needed. \nIn this demo, you will use two credentials to prove who you are and access court materials online instead of in-person.',
      image: '/public/lawyer2/onboarding/scan.svg',
    },
    {
      screenId: 'GOING_DIGITAL',
      title: 'Going Digital',
      text: 'Over time,The Law Society of Ayrotek has been making things more convenient by offering a choice of accessing services online and in-person. Ayşe has accumulated all physical credentials and has gone digital. She has been verified as a practising lawyer in verified herself with the Card app.',
      image: '/public/lawyer2/onboarding/goingDigital.svg',
    },
    {
      screenId: 'ACCESS_COURT_MATERIALS',
      title: 'Accessing court materials',
      text: "Ayşe has been going to the courthouse in person to access voice recordings and other confidential materials. Court Services Branch has let her know that she now has the choice of accessing court materials online. She just needs to prove she's a practising lawyer from Ayrotek and have a government issued ID with a matching name. She can get a digital lawyer member card from the Law Society of Ayrotek and her Person credential from her AyroID Services Card app.",
      image: '/public/lawyer2/onboarding/loginLSBC.svg',
    },
    {
      screenId: 'CONNECT_LSBC',
      title: 'Get your lawyer credential',
      text: 'Ayşe is now ready to be issued her Law Society of Ayrotek Member Card. She has logged into her member portal and is ready to accept a digital version of that card. Open the AyroID app on your phone, hit the scan button and accept.',
      image: '/public/lawyer2/onboarding/lawyerCredPhone.svg',
      issuer_name: 'Law Society of AyroID (Demo)',
    },
    {
      screenId: 'ACCEPT_LSBC',
      title: 'Accept your lawyer credential',
      text: 'Check your phone. You’ve received a credential offer from the Law Society of Ayrotek in your AyroID. You can use this credential to prove you’re a lawyer online.',
      image: '/public/lawyer2/onboarding/lawyerCredPhone.svg',
      credentials: [
        {
          name: 'member_card',
          version: '1.54',
          icon: '/public/lawyer2/connection/lsbc-logo.png',
          attributes: [
            {
              name: 'Member Status',
              value: 'Active',
            },
            {
              name: 'Given Name',
              value: 'Ayşe',
            },
            {
              name: 'PPID',
              value: 'MC12349',
            },
            {
              name: 'Member Status Code',
              value: '1',
            },
            {
              name: 'Surname',
              value: 'Özdemir',
            },
          ],
        },
      ],
    },
    {
      screenId: 'CONNECT_PERSON',
      title: 'Get Person credential',
      text: 'Ayşe gets her Person credential from the AyroID Services Card app. She starts the process within AyroID. For this demo you will scan this QR code to receive the credential offer.',
      image: '/public/lawyer2/onboarding/personCredPhone.svg',
      issuer_name: 'Service Ayrotek (Demo)',
    },
    {
      screenId: 'ACCEPT_PERSON',
      title: 'Accept your Person credential',
      text: 'Check your phone. You’ve received a credential offer from Service Ayrotek in your AyroID. You can use this credential to prove who you are online.',
      image: '/public/lawyer2/onboarding/personCredPhone.svg',
      credentials: [
        {
          name: 'Person',
          version: '1.2',
          icon: '/public/lawyer2/connection/bc-logo.png',
          attributes: [
            {
              name: 'postal_code',
              value: 'V8N2X6',
            },
            {
              name: 'picture',
              value:
                'data:image/png;base64,',
            },
            {
              name: 'given_names',
              value: 'Ayşe',
            },
            {
              name: 'family_name',
              value: 'Özdemir',
            },
            {
              name: 'locality',
              value: 'City',
            },
            {
              name: 'region',
              value: 'Ataşehir',
            },
            {
              name: 'street_address',
              value:
                'Küçükbakkalköy Mah. Dudullu Yolu Cad. No:23-25B Brandium Rezidans R2 Blok K:9 D:87-88, 34750 Ataşehir/İstanbul',
            },
            {
              name: 'country',
              value: 'Turkey',
            },
            {
              name: 'expiry_date_dateint',
              value: `${getDateInt(5)}`,
            },
            {
              name: 'birthdate_dateint',
              value: `${getDateInt(-19)}`,
            },
          ],
        },
      ],
    },
    {
      screenId: 'SETUP_COMPLETED',
      title: "You're all set!",
      text: 'Congratulations, you’ve just received your first digital credentials. They are safely stored in your wallet and ready to be used. So, what are you waiting for? Let’s go!',
      image: '/public/lawyer2/onboarding/lawyer2Success.svg',
    },
  ],
  useCases: [
    {
      id: 'courtServices',
      name: 'Court Services',
      screens: [
        {
          screenId: 'START',
          title: 'Gain access to court materials online',
          text: 'Ayşe can gain entry to Access to Court Materials, a service online by Court Services Branch where lawyers can access court documents online. They require proof that you’re a practising lawyer in Ayrotek and a matching name to your lawyer member card.',
          image: '/public/lawyer2/useCases/courtServices/bothCreds.svg',
        },
        {
          screenId: 'CONNECTION',
          title: 'Start proving you’re a lawyer',
          text: 'As Ayşe you’re now ready to prove you’re a practising lawyer your name to Court Services Branch and gain entry to Access to Court Materials online. Scan the QR code.',
          image: '/public/lawyer2/useCases/courtServices/courtServicesOverlay.png',
          verifier: { name: 'Court Services Branch (DEMO)', icon: '/public/lawyer2/connection/lsbc-logo.png' },
        },
        {
          screenId: 'PROOF',
          title: 'Confirm the information to send',
          text: 'AyroID will now ask you to confirm what to send. Notice how you’re not sharing your entire credential. Court Services Branch is requesting that you prove only what is needed.',
          requestOptions: {
            title: 'Court Services Branch Request',
            text: 'Court Services Branch would like some of your personal information.',
            requestedCredentials: [
              {
                icon: '/public/lawyer2/connection/lsbc-logo.png',
                name: 'member_card',
                properties: ['Given Name', 'Surname', 'PPID'],
              },
              {
                icon: '/public/lawyer2/connection/bc-logo.png',
                name: 'Person',
                properties: ['given_names', 'family_name'],
              },
            ],
          },
        },
        {
          screenId: 'STEP_END',
          title: "You're done!",
          text: 'You’ve proved to Court Services Branch that you’re a practising lawyer from your identity using your Person credential. You can now access court materials online from the comfort of your own home. It only took a few seconds and you revealed minimal information that Court Services Branch could easily and automatically trust.',
          image: '/public/lawyer2/onboarding/lawyer2Success.svg',
        },
      ],
    },
    {
      id: 'courtLibrary',
      name: 'Court Library',
      screens: [
        {
          screenId: 'START',
          title: 'Gain access to the Court Library in person',
          text: 'Ayşe wants to gain access to the court library using her digital credentials. The court library requires that Ayşe provides photo identification.',
          image: '/public/lawyer2/useCases/courtServices/bothCreds.svg',
        },
        {
          screenId: 'CONNECTION',
          title: 'Start proving you’re a lawyer',
          text: 'As Ayşe you’re now ready to prove you’re a practising lawyer to the court library. Scan the QR code.',
          image: '/public/lawyer2/useCases/courtServices/courtServicesOverlay.png',
          verifier: { name: 'Court Library', icon: '/public/lawyer2/connection/lsbc-logo.png' },
        },
        {
          screenId: 'PROOF',
          title: 'Confirm the information to send',
          text: 'AyroID will now ask you to confirm what to send. Notice how you’re not sharing your entire credential. The Court Library is requesting that you prove only what is needed.',
          requestOptions: {
            title: 'Court Library Request',
            text: 'Court Library would like some of your personal information.',
            requestedCredentials: [
              {
                icon: '/public/lawyer2/connection/lsbc-logo.png',
                name: 'member_card',
                properties: ['Given Name', 'Surname', 'PPID'],
              },
              {
                icon: '/public/lawyer2/connection/bc-logo.png',
                name: 'Person',
                properties: ['given_names', 'family_name', 'picture'],
              },
            ],
          },
        },
        {
          screenId: 'STEP_END',
          title: "You're done!",
          text: 'You’ve proved to the Court Library that you’re a practising lawyer identity using your Person credential. You can now access court materials online from the comfort of your own home. It only took a few seconds and you revealed minimal information that Court Services Branch could easily and automatically trust.',
          image: '/public/lawyer2/onboarding/lawyer2Success.svg',
        },
      ],
    },
  ],
}
